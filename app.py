...
# Utility: Retrieve top-k results by cosine similarity
def find_similar_images(query_embedding, num_results=5):
    ...
    similarities = embeddings_db['embedding'].apply(
        lambda stored_embedding: F.cosine_similarity(
            query_embedding, torch.tensor(stored_embedding), dim=0
        ).item()
    )
...
def perform_search():
    ...
    if query_type == "text":
        ...
        tokenized_text = text_tokenizer([user_text])
        query_embedding = F.normalize(clip_model.encode_text(tokenized_text)).squeeze(0)

    elif query_type == "image":
        uploaded_image = request.files["image_query"]
        processed_image = image_preprocessor(Image.open(uploaded_image)).unsqueeze(0)
        query_embedding = F.normalize(clip_model.encode_image(processed_image)).squeeze(0)
        ...
        query_embedding = F.normalize(weight * text_embedding + (1 - weight) * image_embedding).squeeze(0)
